...
--Determine the number of unique 'Outcome' values and their respective counts.
...
-- Find the total number of customers with and without car insurance.
...
-- Create a bucketed table on 'Age', bucketed into 4 groups (as perthe age groups mentioned above). Load data from the original table into this bucketed table.
...
-- If you want to add an additional partition on 'Job' to thepreviously created partitioned table, you actually have to create a new table as Hive does not allow altering the partitioning of existing tables. However, it's a straightforward task to create a new partitioned table and move the data accordingly.
...
-- Calculate the ratio of the number of customers with car insurance to the number of customers without car insurance for each job category.
